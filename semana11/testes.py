...
class alunoTest(unittest.TestCase):
  ...
  def test_salvarAluno(self):   
    resultado = self.aluno.salvar(conexao=self.conexao, colecao='alunos')
    self.assertEqual(True, resultado, 'Aluno não foi salvo corretamente!')

    aluno_inserido = self.conexao['alunos'].find_one({'nome': 'Fabio'})
    self.assertIsNotNone(aluno_inserido, 'Aluno não encontrado na coleção!')
    self.assertEqual(aluno_inserido['nota'], 10, 'Nota do aluno incorreta!')
